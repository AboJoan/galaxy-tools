...
# =========================
: "${EGG_DIR:=00_input/01_eggNOG_functional_annotation/02_plasmid_protein_genes}"   # where PLRe-annotated_XX.csv +  PLRef_NCBI_mapped_XX.csv live
: "${FAA_DIR:=00_input/01_eggNOG_functional_annotation/02_plasmid_protein_genes}"  # where PLRe-annotated_XX.faa +  PLRef_NCBI_mapped_XX.faa live (set properly if different)
: "${OBO_FILE:=00_input/go-basic.obo}"

: "${OUT_TSV:=04_results/01_final_tables/02_Plasmid_sequences_functional_summary_table.tsv}"
: "${OUT_CSV:=04_results/01_final_tables/02_Plasmid_sequences_functional_summary_table.csv}"
...
EGG_DIR = Path(os.environ.get("EGG_DIR", "00_input/01_eggNOG_functional_annotation/02_plasmid_protein_genes"))
FAA_DIR = Path(os.environ.get("FAA_DIR", str(EGG_DIR)))
OBO_FILE = Path(os.environ.get("OBO_FILE", "00_input/go-basic.obo"))

OUT_TSV = Path(os.environ.get("OUT_TSV", "04_results/01_final_tables/02_Plasmid_sequences_functional_summary_table.tsv"))
OUT_CSV = Path(os.environ.get("OUT_CSV", "04_results/01_final_tables/02_Plasmid_sequences_functional_summary_table.csv"))
...
def detect_indices() -> list[str]:
    new_csv = set()
    old_csv = set()
    for p in EGG_DIR.glob("PLRe-annotated_*.csv"):
        m = re.search(r"PLRe-annotated_(\d+)\.csv$", p.name)
        if m:
            new_csv.add(m.group(1))
    for p in EGG_DIR.glob("PLRef_NCBI_mapped_*.csv"):
        m = re.search(r"PLRef_NCBI_mapped_(\d+)\.csv$", p.name)
        if m:
            old_csv.add(m.group(1))

    new_faa = set()
    old_faa = set()
    for p in FAA_DIR.glob("PLRe-annotated_*.faa"):
        m = re.search(r"PLRe-annotated_(\d+)\.faa$", p.name)
        if m:
            new_faa.add(m.group(1))
    for p in FAA_DIR.glob("PLRef_NCBI_mapped_*.faa"):
        m = re.search(r"PLRef_NCBI_mapped_(\d+)\.faa$", p.name)
        if m:
    ...
    if not common:
        die(
            ...
            f"  FAA_DIR={FAA_DIR}\n"
            f"Need pairs like:\n"
            f"  PLRe-annotated_01.csv +  PLRef_NCBI_mapped_01.csv\n"
            f"  PLRe-annotated_01.faa +  PLRef_NCBI_mapped_01.faa\n"
        )
    return common
...
for idx in indices:
    new_id = f"PLRe-annotated_{idx}"
    old_id = f"PLRef_NCBI_mapped_{idx}"
